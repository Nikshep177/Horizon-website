...
const BentoCardGrid = ({ children, gridRef, className = '' }) => (
  <div className={`card-grid magic-bento-section ${className}`.trim()} ref={gridRef}>
    {children}
  </div>
)
...
export default function MagicBento({
  ...
  return (
    <>
      ...
      <BentoCardGrid gridRef={gridRef} className={containerClassName}>
        {(Array.isArray(children) ? children : [children]).map((child, index) => {
          const baseClassName = `magic-bento-card ${className} ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''}`
