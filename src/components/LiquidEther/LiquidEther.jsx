...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class WebGLManager {
      ...
      loop() { if (!this.running) return; this.render(); requestAnimationFrame(this._loop); }
      ...
      dispose() { try { window.removeEventListener('resize', this._resize); document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose(); const canvas = Common.renderer.domElement; if (canvas && canvas.parentNode) canvas.parentNode.removeChild(canvas); Common.renderer.dispose(); Common.renderer.forceContextLoss(); } catch { void 0; } }
    }
